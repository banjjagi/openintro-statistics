...
\index{사건|(}
통계학자는 거의 개별 결과로 작업하지 않고 대신에 결과 \indexthis{\emph{집합}}{sets}(set) or \indexthis{\emph{모임}}{collections}(collections)을 고려한다. $A$가 주사위를 굴렸을 때 \resp{1} 혹은 \resp{2}가 나올 사건을 나타내고, $B$가 주사위 굴렸을 때 \resp{4} 혹은 \resp{6} 이 나올 사건을 나타낸다고 하자. $A$ 를 $\{$\resp{1},~\resp{2}$\}$ 의 결과 집합으로 적고, $B=\{$\resp{4}, \resp{6}$\}$ 와 같이 적는다. 이러한 집합을 일반적으로 \termsub{사건}{event}(events)으로 불린다. $A$ 와 $B$ 는 공통된 요소가 없기 때문에, 서로 겹치지 않는 사건이다. $A$ 와 $B$ 가 그림~\ref{disjointSets}에 나타나 있다.
